...
,base_data as (
    ...
    --IT_OPERATION (Kullanıcı Destek) eforları
    union all
    select
        period,
        trx_date,
        ...
        author_name,
        issue_type_name,
        'expensed' as capex_opex,
        'Inhouse' as is_outsource_inhouse,
        '-' || project_id as project_id,
        'IT_OPERATION (Kullanıcı Destek)' as project_name,
        time_spent_seconds,
        logged_time_kg,
        'N/A' as category,
        tribe,
    ...
    where project_name = 'IT_OPERATION'
      ...
      and period >= '2025-08' and period < '2026-01'  
    union all
    select
        period,
        trx_date,
...
--select * from base_data where lower(project_name) like '%kullanıcı%destek%'
,tribe_efforts as (
    select
        period,
...
(
    ...
    left join capex_opex_weights cow
        ...
        and de.project_name = cow.project_name
    where 1 = 1
    and de.category not in ('Board')
)
-- select * from final
-- Problemli projelerin AYLIK total_distributed_all toplamını hesapla
,problemli_projeler_toplam as 
(
    select
        period,
        round(sum(total_distributed_all), 8) as problemli_toplam,
        round(sum(total_distributed_all) / 4.0, 8) as pay_per_hedef_proje
    from final
    where project_name in (
        'LOYALTY_ZUBIZU_Maintenance',
        'WALLET_KOSGEB',
        'WALLET_DGPARA_Maintenance',
        'WALLET_OPET_Maintenance',
        'ANDROIDPOS_DGPARA_Maintenance')
    group by period
)
--select * from problemli_projeler_toplam
,final_adjusted as 
(
    select
        f.*,
        -- Problemli Projeler
        pp.problemli_toplam,
        pp.pay_per_hedef_proje,
        case
            -- Problemli projeler: Temmuz ve sonrası için 0
            when f.project_name in (
                'LOYALTY_ZUBIZU_Maintenance',
                'WALLET_KOSGEB',
                'WALLET_DGPARA_Maintenance',
                'WALLET_OPET_Maintenance',
            ...
            -- Hedef projeler: Normal değer + problemli projelerden gelen pay
            when f.project_name in (
                'COREBANKING_EMLAK_KATILIM',
                'PREPAIDCARD_PTT_Maintenance',
                'COREBANKING_PTT',
                'PREPAIDCARD_TOHAS_Maintenance'
            ) then round(f.total_distributed_all + coalesce(pp.pay_per_hedef_proje, 0), 8)
            -- Diğer tüm durumlar: Normal değer
            else f.total_distributed_all
        end as total_distributed_all_adjusted,
        -- Finansal V2
        case
            -- Problemli projeler: Temmuz ve sonrası için sadece distributed_total_proje_team_efforts - exclude_effort
            when f.project_name in (
                'LOYALTY_ZUBIZU_Maintenance',
                'WALLET_KOSGEB',
                'WALLET_DGPARA_Maintenance',
                'WALLET_OPET_Maintenance',
                'ANDROIDPOS_DGPARA_Maintenance'
            ) then f.distributed_total_proje_team_efforts + 0 - f.exclude_effort
            -- Hedef projeler: Normal finansal + problemli projelerden gelen pay
            when f.project_name in (
                'COREBANKING_EMLAK_KATILIM',
                'PREPAIDCARD_PTT_Maintenance',
                'COREBANKING_PTT',
                'PREPAIDCARD_TOHAS_Maintenance'
            ) then round(f.final_effort + coalesce(pp.pay_per_hedef_proje, 0), 8)
            -- Diğer tüm durumlar: Normal finansal
            else f.final_effort
        end as final_effort_adjusted
    from final f
    left join problemli_projeler_toplam pp
        on f.period = pp.period
    where 1=1
    ----debug1
    --Problemli propjeleri 0 gelecek olanlar
    --and f.project_name not in (
    --    'LOYALTY_ZUBIZU_Maintenance',
    --    'WALLET_KOSGEB',
    --    'WALLET_DGPARA_Maintenance',
    --   'WALLET_OPET_Maintenance',
...
(
    select de.*,
            ...
            de.final_effort_adjusted - coalesce(iow.outsource_effort, 0) as final_effort_inhouse,
            -- Outsource = Ham outsource eforu (dağıtılmamış)
            coalesce(iow.outsource_effort, 0) as final_effort_outsource
    from final_adjusted de
