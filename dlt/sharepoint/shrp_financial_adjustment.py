"""
SharePoint Financial Adjustment DLT Pipeline

Extracts data from "financial_adjustment.xlsx" Excel file stored in SharePoint Document Library
and loads it to PostgreSQL.

The Excel file contains monthly adjustment data that needs to be loaded into raw_sharepoint schema.
"""
...
SHAREPOINT_FILE_PATH = "/financial_adjustment.xlsx"  # Path relative to the drive
SHEET_NAME = "adjustment"  # Sheet name in Excel file
TABLE_NAME = "financial_adjustment"
SCHEMA_NAME = "raw_sharepoint"
PIPELINE_NAME = "shrp_financial_adjustment"
...
def download_excel_file_from_sharepoint(site_id: str, drive_name: str, file_path: str, access_token: str) -> bytes:
    ...
    Args:
        ...
        file_path: Path to the file relative to the drive (e.g., "/financial_adjustment.xlsx")
...
def create_financial_adjustment_resource(write_disposition: TWriteDisposition):
    """Factory function to create dlt resource with dynamic write disposition."""
    @dlt.resource(
        name=TABLE_NAME,
        write_disposition=write_disposition,
        columns={
            "project_id": {"nullable": True, "data_type": "bigint"},
            "year": {"nullable": False, "data_type": "bigint"}
        }
    )
    def financial_adjustment_resource():
        """dlt resource for SharePoint financial adjustment Excel file."""
        ...
        # Note: We no longer filter NULL project_id here - let dbt handle business logic
        # This is consistent with capex_opex_adjustment and operation_efforts pipelines
        for record in records:
            record['_etl_date'] = datetime.now().isoformat()
            yield record

        print(f"✅ Yielded {len(records)} records to pipeline")

    return financial_adjustment_resource


def run_pipeline(mode: str = "initial"):
    """Run the SharePoint Financial Adjustment pipeline."""
    print("=" * 80)
    print(f"SharePoint Financial Adjustment Pipeline - Mode: {mode.upper()}")
    print("=" * 80)
    ...
    # Financial adjustment data is always a complete snapshot, not incremental
    ...
    # This ensures clean state management between runs and allows schema evolution
    ...
    financial_adjustment_res = create_financial_adjustment_resource(write_disposition)
    data = financial_adjustment_res()
    ...
    print("\n" + "=" * 80)
    print("✅ SharePoint Financial Adjustment Load Completed!")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SharePoint Financial Adjustment DLT Pipeline")
