"""
SharePoint Issue Type DLT Pipeline

Extracts all items from the "Issue Type" SharePoint list and loads them to PostgreSQL.
...
# ===== CONFIGURATION =====
SHAREPOINT_LIST_TITLE = "Issue Type"
TABLE_NAME = "issue_type"
SCHEMA_NAME = "raw_sharepoint"
PIPELINE_NAME = "shrp_issue_type"


def create_issue_type_resource(write_disposition: TWriteDisposition):
    ...
    def issue_type_resource():
        """dlt resource for SharePoint Issue Type list."""
    ...
    return issue_type_resource


def run_pipeline(mode: str = "initial"):
    """Run the SharePoint Issue Type pipeline."""
    print("=" * 80)
    print(f"SharePoint Issue Type Pipeline - Mode: {mode.upper()}")
    print("=" * 80)
    ...
    issue_type_res = create_issue_type_resource(write_disposition)
    data = issue_type_res()
    ...
    print("\n" + "=" * 80)
    print("✅ SharePoint Issue Type Load Completed!")
    print("=" * 80)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SharePoint Issue Type DLT Pipeline")
