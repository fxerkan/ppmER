"""
Jira Issue Links DLT Pipeline - OPTIMIZED VERSION

HIGH-PERFORMANCE extraction with:
- Parallel processing with configurable workers (default: 8)
- Connection pooling with requests.Session
- Concurrent issue link fetching for multiple issues
...
Pipeline Modes:
    ...
    - daily: Extract links for issues updated in last N days

Both modes use replace since issue links are extracted fresh each time.

Usage:
    docker exec ppm-dlt python /app/jira/jira_issue_links_optimized.py --mode=initial
    docker exec ppm-dlt python /app/jira/jira_issue_links_optimized.py --mode=daily
    docker exec ppm-dlt python /app/jira/jira_issue_links_optimized.py --mode=initial --max-workers=16
"""
...
PIPELINE_NAME = "jira_issue_links_optimized"
...
class ProgressTracker:
    ...
    def update(self, links_count: int):
        with self.lock:
            self.issues_processed += 1
            self.links_collected += links_count
            return self.issues_processed, self.links_collected
...
def fetch_issues_for_date_range(
    ...
    """Fetch issues for a specific date range with issuelinks using session pooling"""
...
def extract_issue_links_from_single_issue(issue: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract issue links from a single issue - designed for parallel processing"""
    ...
    for link in issue_links:
        ...
        link_record = {
            'source_issue_key': issue.get('key'),
            'link_type_name': link_type.get('name'),
            'link_type_inward': link_type.get('inward'),
            'link_type_outward': link_type.get('outward'),
        }

        if 'outwardIssue' in link:
            link_record['direction'] = 'outward'
            link_record['target_issue_key'] = link['outwardIssue'].get('key')
            link_record['target_summary'] = link['outwardIssue'].get('fields', {}).get('summary')
        elif 'inwardIssue' in link:
            link_record['direction'] = 'inward'
            link_record['target_issue_key'] = link['inwardIssue'].get('key')
            link_record['target_summary'] = link['inwardIssue'].get('fields', {}).get('summary')

        link_record['_etl_date'] = datetime.now().isoformat()
        links.append(link_record)

    return links


def extract_issue_links_parallel(issues: List[Dict[str, Any]], max_workers: int) -> List[Dict[str, Any]]:
    """Extract issue links from multiple issues using PARALLEL processing"""
    if not issues:
        return []

    print(f"\nProcessing issue links...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_issue = {
            executor.submit(extract_issue_links_from_single_issue, issue): issue
            for issue in issues
        }
        ...
        for future in as_completed(future_to_issue):
            try:
                ...
                issues_processed, links_collected = tracker.update(len(links))

                # Progress reporting
                if issues_processed % BATCH_SIZE == 0 or issues_processed == len(issues):
                    elapsed = time.time() - start_time
                    rate = issues_processed / elapsed if elapsed > 0 else 0
                    print(f"   Progress: {issues_processed}/{len(issues)} issues, {links_collected} links ({rate:.1f} issues/sec)")
                    sys.stdout.flush()
    ...
    print(f"   Completed: {len(issues)} issues in {elapsed:.1f}s ({rate:.1f} issues/sec, {len(all_links)} links)")
    sys.stdout.flush()
...
def run_pipeline(mode: str = "initial", max_workers: int = MAX_WORKERS):
    """Run the OPTIMIZED Jira issue links pipeline."""
    print("=" * 80)
    print(f"Jira Issue Links Pipeline (OPTIMIZED) - Mode: {mode.upper()}")
    print("=" * 80)
    ...
    # Extract links using PARALLEL processing
    all_links = extract_issue_links_parallel(raw_issues, max_workers)

    print(f"\nExtracted {len(all_links)} issue links")
    sys.stdout.flush()
    ...
    load_info = pipeline.run(
        dlt.resource(all_links, name=TABLE_NAME, write_disposition="replace")
    )
    ...
    print("\n" + "=" * 80)
    print("Jira Issue Links Load Completed (OPTIMIZED)!")
    print("=" * 80)
    print(f"   Pipeline: {PIPELINE_NAME}")
    print(f"   Table: {SCHEMA_NAME}.{TABLE_NAME}")
    print(f"   Records: {len(all_links)}")
    print(f"   Mode: {mode}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Jira Issue Links DLT Pipeline (OPTIMIZED)")
